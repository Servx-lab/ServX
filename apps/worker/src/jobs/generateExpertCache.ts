...
import { ENCRYPTION_KEY_RAW } from '@servx/crypto';

// Re-using the same path logic but adjusted for the new location if needed.
// Original: path.join(__dirname, '../server/data/errorCache.json')
// New location is apps/worker/src/jobs/generateExpertCache.ts
// So we need to go up 4 levels to reach root, then into server/data
const CACHE_FILE_PATH = path.join(process.cwd(), 'server/data/errorCache.json');
...
const buildHttpError = (status: { code: number; name: string }): ExpertEntry => ({
...
const buildSystemError = (err: { code: string; desc: string }): ExpertEntry => ({
...
const buildMongoError = (mongo: { code: number; name: string }): ExpertEntry => ({
...
const buildEdgeCase = (item: { name: string; pattern: string; fix: string }): ExpertEntry => ({
...
export const generateExpertCache = async () => {
    ...
    let cache: Record<string, ExpertEntry> = {};
    ...
    // Synthetic Mongo loop for codes 1-100
    for (let i = 1; i <= 100; i++) {
