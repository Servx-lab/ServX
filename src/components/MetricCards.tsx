...
const MetricCard = ({ value, label, change, changePositive = false, accentColor, delay }: MetricCardProps) => (
  <motion.div
    className="glass-card px-5 py-4 flex items-baseline gap-3"
  ...
  >
    ...
    <span className={`text-xs font-mono ${changePositive ? "text-emerald-400" : "text-destructive"}`}>
      {change}
    </span>
    <span className="text-muted-foreground text-sm ml-auto">{label}</span>
  </motion.div>
);
