...
2. **Server-Sent Events Telemetry (apps/api)**:
   - real-time stream `GET /security/scan/stream/:scanId` pushing events: logs, nodes, and findings.
...
|---|---|---|---|---|
| 0 | E2E Testing Track | Define test infra, implement Tiers 1-4, publish `TEST_READY.md` | None | IN_PROGRESS (Conv: 5e043b0f-7ebb-4a0b-a084-98fc5454f8b0) |
...
| 3 | Telemetry SSE Stream | SSE streaming endpoint `GET /security/scan/stream/:scanId` | M2 | PLANNED |
...
### API Scan ↔ SSE Stream
- **Interface**: `GET /security/scan/stream/:scanId`
...
## Code Layout
- `apps/api/src/domains/security/` - Backend scan controllers, router, and telemetry streams
